...
def cli4(args):
    ...
    usage = 'usage: cli4 [-h|--help] [-v|--verbose] [-q|--quiet] '\
        '[--get|--patch|--post|-put|--delete] [item=value ...] /command...'

    try:
        opts, args = getopt.getopt(args, 'hvq',
                                   ['help', 'verbose', 'quiet', 'get',
    ...
    # bread down command into it's seperate pieces; these are then checked
    # against the CloudFlare class to confirm there is a method that matches
    parts = command.split('/')
        ...
        if element[0] == ':':
            ...
            if identifier1 is None:
                if len(element) in [32, 40, 48] and hex_only.match(element):
                    ...
                    identifier1 = convert_virtual_dns_to_identifier(cf,
                                                                    element)
                else:
